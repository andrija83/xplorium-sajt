...
/**
 * Sensory Room Route Page
 *
 * Dedicated route for the sensory room section with:
 * - Planet orb navigation (Floor, Wall, Ceiling)
 * - Gallery views for each subsection
 * - Same visual design as main landing page
 */
export default function SensoryPage() {
  const router = useRouter()
  const { sensorySubView } = useNavigationStore()
  ...
  return (
    <div className="min-h-screen w-full bg-black overflow-hidden">
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[100] focus:px-4 focus:py-2 focus:bg-white focus:text-black focus:rounded-md focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-black"
      >
        Skip to main content
      </a>

      {/* Starfield Background */}
      <Starfield activeView="discover" />
      ...
      <AnimatePresence>
        <motion.button
          onClick={() => sensorySubView ? router.back() : router.push('/')}
          onKeyDown={(e) => {
          ...
          aria-label={`Go back to ${sensorySubView ? 'sensory menu' : 'main menu'}`}
          tabIndex={0}
